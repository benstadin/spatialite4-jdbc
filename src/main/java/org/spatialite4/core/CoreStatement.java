...
import org.spatialite4.jdbc4.JDBC4ResultSet;
...
public abstract class CoreStatement implements Codes
{
    ...
    protected boolean exec() throws SQLException {
        if (sql == null)
            throw new SQLException("SpatiaLite4 internal error: sql==null");
        if (rs.isOpen())
            throw new SQLException("SpatiaLite4 JDBC internal error: rs.isOpen() on exec.");
    ...
    protected boolean exec(String sql) throws SQLException {
        if (sql == null)
            throw new SQLException("SpatiaLite4 internal error: sql==null");
        if (rs.isOpen())
            throw new SQLException("SpatiaLite4 internal error: rs.isOpen() on exec.");
